...
const Expertises = () => {
    ...
    return (
        <section className="pb-6 md:pb-8 lg:pb-12">
            { data && data.expertises?.title && <h2 className="text-h3 font-bold text-center pb-6">{data.expertises?.title}</h2>}
            { data && data.expertises?.description &&
            ...
            <ScrollTrigger onEnter={() => setCounterStart(true)}>
                { data && data.expertises?.counter.length > 0 &&
                    <div
                    className="bg-accent rounded-lg grid grid-cols-2 lg:grid-cols-4 w-fit mx-auto gap-8 p-8 text-white">
                    {data.expertises.counter.map((item: ExpertiseType) => (
                        <div className="flex flex-col items-center justify-center px-8 text-center" key={item.id}>
                            <span className="font-bold text-h3">{counterStart &&
                                <CountUp start={0} duration={1.5} end={item.number}/>}</span>
